"""Local test harness for Episode 8 — Autonomous Agents.

Three modes:
  python episodes/ep-08-autonomous-agents/preflight.py --plan       # Emit human-readable test plan (markdown)
  python episodes/ep-08-autonomous-agents/preflight.py --run        # Pre-flight + non-destructive smoke (no T1)
  python episodes/ep-08-autonomous-agents/preflight.py --trigger    # Adds T1: ephemeral task lifecycle (CREATE/MODIFY/POLL/DELETE)
  python episodes/ep-08-autonomous-agents/preflight.py --plan --run # Both: show plan, then run

Sentinel cannot be programmatically invoked any more than the Ep 7 chat agent.
...
Pre-flight (substrate):
  ...
  P3: lc_CalculateLaunchReadiness Custom API exists (reused by Coordinator + Ep 8 Part 2)
...
when it's deployed) -> episodes/ep-08-autonomous-agents/prompts.md.
"""
import argparse
import json
import os
...
PLAN = """# Episode 8 - Local Test Plan

> Episode 8 builds Launch Sentinel (autonomous Copilot Studio agent that
...
- Manual prompt set written to `episodes/ep-08-autonomous-agents/prompts.md`
...
def render_console(results):
    print()
    print(C.BOLD + "Episode 8 - Local Test Run" + C.END)
    print("=" * 60)
...
PROMPT_DOC = """# Episode 8 - Manual Verification Checklist
