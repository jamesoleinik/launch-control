...
Agents have spread through the campaign: the Copilot Studio agent (now Ep 11, the Web IQ build), the Sentinel autonomous runtime (now Ep 10, Fabric IQ), the code-first agent (now Ep 12, Foundry IQ). An admin needs the **standing report**: what tables, what actions, by which agent — without crawling each one's settings page.
