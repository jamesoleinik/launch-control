...
Three modes:
  python scripts/test_ep7_locally.py --plan       # Emit human-readable test plan (markdown)
  python scripts/test_ep7_locally.py --run        # Pre-flight + non-destructive smoke (no T1)
  python scripts/test_ep7_locally.py --trigger    # Adds T1: ephemeral task lifecycle (CREATE/MODIFY/POLL/DELETE)
  python scripts/test_ep7_locally.py --plan --run # Both: show plan, then run
...
when it's deployed) -> scripts/test_ep7_prompts.md.
"""
import argparse
import json
import os
...
- Manual prompt set written to `scripts/test_ep7_prompts.md`
...
_Generated by `scripts/test_ep7_locally.py`. Re-run after substrate changes._
