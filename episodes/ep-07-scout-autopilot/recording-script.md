...
Target length: **~2:05**. One MCP-shape beat (Part 0), three demo parts, plus an 8s split-screen intro and a 5s outro. Same screen-recording-with-voiceover format as Episodes 1–6.
...
| **0:08–0:25** ⭐ **Part 0 · The new MCP shape, discovered live (17s)** | Scout chat fullscreen. James pastes the Part 1a discovery prompt. Scout renders the area-grouped tool catalog inline. Then the HTML server visual opens and lists tool names vertically. Hold at 1x. Cut tight when the visual opens. | "First, what changed. I had Scout inventory the new Dataverse MCP server live. Sixteen tools across five areas, plus three new agentic tools that let the agent search inside attached files and fire server-side logic on its own." | ⬇ **New shape. New agentic tools (search, invoke API & prompt).** |
| **0:25–0:58** ⭐ **Part 1 · Co-author the Business Skill with Scout, then save it (33s)** | Scout chat. James pastes the Part 1b seed prompt (short, no tool names, but with the dedup-via-read-then-read-inside rule called out). Scout drafts the skill body inline; markdown scrolls. James types one iteration. Scout amends. James types "Save it." Tool-use panel fires four MCP calls: `upsert_skill` → `create_skill_resource` → `init_file_upload` → `commit_file_upload`. Hard cut to Power Apps → Skill table → the new `Launch Readiness Sweep` row → the resource attached. | "Then I told Scout what I wanted: sweep my Outlook and Teams for launch issues, and never file a duplicate. Ask the server if any open task already covers the finding, including the content inside the attached PDFs. Scout drafted it, we tightened one rule, I said save it. Four MCP calls. The skill is a row in Dataverse, next to the data it governs." | ⬇ **New skill co-authored with Scout. Saved as a business skill to Dataverse.** |
| **0:58–1:33** ⭐ **Part 2 · Prove `search_data` solo, then run the skill (35s)** | Scout chat. James pastes the Step 2a natural-language question. Tool-use panel: one call, `search_data`. Result panel shows three different `lc_task` row paths for Q3 Widget Launch about the same export-to-CSV crash, plus a matched-content excerpt quoted from inside the attached PDF on the one row that has collateral. Hold at 1x on the row paths and the excerpt. Then James pastes `Run the Launch Readiness Sweep against Q3 Widget Launch.` Tool-use panel: `search`/`describe` loads the skill, then it executes. Per finding, **`search_data`** fires once and returns matching row paths plus the excerpt that triggered the match. Then `update_record` + the file-upload trio per task touched. Speed-ramp 3x through the panel; drop to 1x on the four-section summary, which should call out **1 new, 1 enriched**. James pastes Step 2c. Answer streams a verbatim quote from inside the newly attached PDF via `file_download`. | "Then I asked one question: do I have any open tasks for export issues on Q3? Scout fired `search_data` once and surfaced three duplicates, plus a line from inside an attached PDF. That's what an agent-ready data platform like Dataverse prevents. Then I ran the sweep. One new task, one enriched, zero new duplicates added." | ⬇ **Zero duplicate tasks, zero manual review. All unlocked by the new DV server and agentic search tool.** |
| **1:33–2:00** ⭐ **Part 3 · Always-on Scout (27s)** | Scout → Automations → open "Morning Launch Control update". Edit. Paste the 3 new step bodies (2x speed on the typing). Save. Click Run now. Cut to Teams · the summary DM lands with launch name, the new task names, and the verbatim risk excerpt. | "Then Scout puts it on a schedule. Discover the skill. Run it. Send the summary to Teams. Every weekday at nine." | ⬇ **My launch readiness process, now automated.** |
| **2:00–2:05** | End card. *"Next: Episode 8: RBAC."* `github.com/jamesoleinik/launch-control` | "More agentic clients mean new security exposure on most platforms. Not on Dataverse. Episode 8 next." | ⬇ **More agents, more risk. Not on Dataverse.** |
