"""Episode 16 preflight: verify orchestra demo readiness.

Ep 16 = the 'Full Orchestra + Your Turn' closer. Six checks:
  ...
  4. Capacity API is reachable (Ep 15 carryover -- Ep 16 may show capacity in montage)
...
Run: python episodes/ep-16-full-orchestra/preflight.py
"""
...
def main():
    ...
    spin = os.path.join(os.path.dirname(__file__), 'orchestra', 'spin_up_launch.py')
    try:
    ...
    # ---- 4: capacity API still reachable (carryover Ep 15) ----
    try:
    ...
    print()
    if failures == 0:
        print("ALL GREEN -- Ep 16 orchestra harness ready.")
        print()
