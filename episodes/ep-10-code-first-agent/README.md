# Episode 9 — The Code-First Agent
...
> _"Episode 7 was Copilot Studio. Episode 8 was the autonomous Sentinel. Today's agent? Pure Python. Different runtime, different stack — same brain. Three agents. One source of truth. Edit the skill in Dataverse, all three change."_

Episodes 6 and 7 lived inside Copilot Studio's hosted runtime. Beautiful demos, but viewers naturally ask: _"yeah but I write code — what does this look like in my editor?"_ Episode 9 is the answer. We rebuild the Launch Coordinator as a **code-first Python agent** using:
...
The point isn't to do anything the hosted agents can't. The point is: **the brain is portable**. Skill markdown stored in Dataverse drives behavior across three completely different runtimes — Copilot Studio chat (Ep 7), Copilot Studio autonomous (Ep 8), and now a 250-line Python program (Ep 9).
...
The **brain-sync table** is the Episode 9 money shot. Every line in that table came from a `GET /api/data/v9.2/skills` call against the Dataverse environment, two seconds before the agent started. Edit a skill in Dataverse, re-run, the line counts shift — and so does the verdict downstream.
...
| --- | --- | --- | --- | --- |
| **Copilot Studio chat** | Ep 7 | `make.powerautomate.com` UI | M365 user | MCP `describe('skills/...')` |
| **Copilot Studio autonomous** | Ep 8 | Same UI, autonomous binding | Service principal (CIB) | MCP `describe('skills/...')` |
| **Python (code-first)** | Ep 9 | VS Code, your editor | GitHub Copilot OAuth + Azure CLI | REST `GET /skills` → `./.skills/*.md` |
...
| 1:00 | First token, then streamed response | "Same logic Ep 7 ran. Same verdict Ep 8 emails." |
...
`episodes/ep-09-code-first-agent/preflight.py` is the preflight equivalent of Ep 8's harness. It verifies:
...
```
python episodes/ep-09-code-first-agent/preflight.py
```
...
**Episode 10 — The Dashboard.** Three agents are populating the same tracker.
