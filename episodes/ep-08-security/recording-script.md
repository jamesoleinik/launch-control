...
runtime payoff in Cowork, plus an 8s split-screen intro and a 5s outro. Recorded
beats: 8s intro · 22s P0 · 25s P1 · 30s P2 · 25s P3 · 25s payoff · 5s outro. Same
...
> *"Last episode I said more agents mean more risk on most platforms. Not on
> Dataverse. This episode I prove it: same agent, same question, and the platform
...
| Moment | Caption |
|---|---|
| Cold open | `Same agent. Same question. The platform decides.` |
| Part 0 | `Two axes: rows and columns. One skill.` |
| Part 1 | `Row-level: one query, four personas, four answers` |
| Part 2 | `Column-level: email masked, name hidden, same row` |
| Part 3 | `Per-agent: effective access = human ∩ agent` |
| Payoff | `Masking on vs off. Enforced on the read, not the agent.` |
| Outro | `Author it once. Every MCP-aware agent inherits it.` |
