...
The demo env must be in this exact state before camera turns on. `scripts/python/orchestra/setup_launch_week.py` enforces it idempotently — re-runnable between takes.
...
4. `python scripts/python/orchestra/spin_up_launch.py "My First Launch" --apply` (~8 sec, sped up to ~3 on screen)
...
```bash
python scripts/test_ep12_locally.py
```
...
| Path | Role |
|---|---|
| `docs/episodes/episode-12.md` | This document |
| `scripts/test_ep12_locally.py` | 12-check preflight harness |
| `scripts/python/orchestra/_common.py` | Shared `DV` client + status-code enums |
| `scripts/python/orchestra/setup_launch_week.py` | Forces "perfect launch week" state |
| `scripts/python/orchestra/teardown_launch_week.py` | Restores "messy mid-launch" for re-rehearsal |
| `scripts/python/orchestra/spin_up_launch.py` | Creates a new launch (teaser + Your Turn) |
