...
| **0:08–0:25** ⭐ **Part 0 · The new MCP shape, discovered live (17s)** | Scout chat fullscreen. James pastes the Part 1a discovery prompt. Scout renders the area-grouped tool catalog inline. Then the HTML server visual opens and lists tool names vertically. Hold at 1x. Cut tight when the visual opens. | "First I asked Scout to inventory the Dataverse MCP server. Sixteen tools across five areas, plus three new agentic tools: `search_data`, `invoke_api`, and `execute_prompt`." | ⬇ **New MCP shape. New agentic tools: `search_data`, `invoke_api`, `execute_prompt`.** |
