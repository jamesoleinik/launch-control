# Episode 16: Full Orchestra + Your Turn
...
**Coding agent:** Every coding agent and surface from Eps 1–15, on one stage
**Runtime:** All of the above

---

> **Hook (Part 1):** *"Fifteen episodes. One launch. Watch every piece play together."*
> **Hook (Part 2):** *"Now it's your turn. Clone, paste, ship."*

The closer. Two parts. Part 1 is the **orchestra montage**: every capability built across Eps 1–15 firing in sequence on the same launch. Part 2 is **Your Turn**: viewer goes from `git clone` to a row in their own Dataverse env in under 30 seconds. The repo flips public the moment this drops.

## Why this matters in the arc

Fifteen episodes have argued *"this is real, look at this slice."* Ep 16 closes the loop with two assertions:

1. **The slices are one system.** When the gen page (Ep 14), the autonomous Sentinel (Ep 10), the M365 Copilot grounding (Ep 14), and the management plane (Ep 15) all light up against the same record, the audience sees the platform, not fifteen feature demos.
...
The demo env must be in this exact state before camera turns on. `episodes/ep-16-full-orchestra/orchestra/setup_launch_week.py` enforces it idempotently, re-runnable between takes.
...
|---|---|---|---|
| 1 | **Gen page dashboard** | Ep 14 | Re-establishing shot. Opens green across the board. *"Same record we built piece by piece, here's the whole picture."* |
| 2 | **Custom action `lc_CalculateLaunchReadiness`** | Ep 5 + Ep 13 | Invoke from terminal via the code-first agent. Returns verdict `GO`, score ≥90. *"The business logic that decides 'ship it' is one call."* |
...
| 5 | **M365 Copilot** | Ep 14 | Exec-style prompt: *"Is the Q3 Widget Launch ready to ship?"* → "All gates passed." *"No agent. Just Copilot grounded in this data."* |
| 6 | **Mark Shipped** | Ep 1 + Ep 14 | Toggle `lc_launchstatus` to `Launched` from chat. Confetti. Gen page repaints. *"And this is what shipped looks like."* |

**Bookend:** cut back to the Ep 1 cold-open spreadsheet chaos shot for 2 sec, then back to the all-green dashboard. Caption: *"From this, to this. Fifteen episodes."*
...
4. `python episodes/ep-16-full-orchestra/orchestra/spin_up_launch.py "My First Launch" --apply` (~8 sec, sped up to ~3 on screen)
...
```bash
python episodes/ep-16-full-orchestra/preflight.py
```
...
- ✅ Capacity / BAP admin API reachable (Ep 15 carryover; capacity may flash in montage)
...
The repo flips public when Ep 16 drops. Audit before that moment:
...
| Path | Role |
|---|---|
| `episodes/ep-16-full-orchestra/README.md` | This document |
| `episodes/ep-16-full-orchestra/preflight.py` | 12-check preflight harness |
| `episodes/ep-16-full-orchestra/orchestra/_common.py` | Shared `DV` client + status-code enums |
| `episodes/ep-16-full-orchestra/orchestra/setup_launch_week.py` | Forces "perfect launch week" state |
| `episodes/ep-16-full-orchestra/orchestra/teardown_launch_week.py` | Restores "messy mid-launch" for re-rehearsal |
| `episodes/ep-16-full-orchestra/orchestra/spin_up_launch.py` | Creates a new launch (teaser + Your Turn) |

No new tables, columns, plugins, actions, or agents. Ep 16 is pure orchestration of what's already shipped.

## Follow-ups (not blockers for the doc; blockers for going public)

- [ ] Tag commits `ep-1` through `ep-14`; push tags
- [ ] Write LinkedIn copy for Ep 16 + the pinned-post version that lives at the top of James's profile through the campaign window
...
- **Ep 14 dashboard:** the re-establishing shot for surface #1 of the montage.
- **Ep 15:** the capacity beat may make a cameo if a frame opens up; otherwise omitted to keep Ep 16 focused on launch state, not env state.
