...
Two SDK gotchas worth flagging out loud — both real, both still relevant
even though our merge sidesteps them:
...
2. GitHub Copilot writes `dataframe_tour.py` from the Part 1 prompt.
   `python scripts/python/dataframe_tour.py` runs end-to-end. Five stops,
   all green. The viewer sees the **Python SDK + pandas** operating on
   real Dataverse data — no export, no ETL job.
...
5. `python scripts/python/promote.py --dry-run` → "77 read, 77 to create."
6. `python scripts/python/promote.py` → "77 created."
7. The unified tables now have rows. Cursor highlights a `lc_task` row's
   `lc_sourcestagingaid` column showing the back-reference lookup
   populated.
8. Re-run promote. `0 created, 77 updated.` Idempotent.
