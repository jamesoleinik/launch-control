"""Local test harness for Episode 9 — Autonomous Agents.

Three modes:
  python episodes/ep-09-autonomous-agents/preflight.py --plan       # Emit human-readable test plan (markdown)
  python episodes/ep-09-autonomous-agents/preflight.py --run        # Pre-flight + non-destructive smoke (no T1)
  python episodes/ep-09-autonomous-agents/preflight.py --trigger    # Adds T1: ephemeral task lifecycle (CREATE/MODIFY/POLL/DELETE)
  python episodes/ep-09-autonomous-agents/preflight.py --plan --run # Both: show plan, then run

Sentinel cannot be programmatically invoked any more than the Ep 8 chat agent.
...
Pre-flight (substrate):
  ...
  P3: lc_CalculateLaunchReadiness Custom API exists (reused by Coordinator + Ep 9 Part 2)
...
when it's deployed) -> episodes/ep-09-autonomous-agents/prompts.md.
"""
import argparse
import json
import os
...
PLAN = """# Episode 9 - Local Test Plan

> Episode 9 builds Launch Sentinel (autonomous Copilot Studio agent that
...
- Manual prompt set written to `episodes/ep-09-autonomous-agents/prompts.md`
...
def render_console(results):
    print()
    print(C.BOLD + "Episode 9 - Local Test Run" + C.END)
    print("=" * 60)
...
PROMPT_DOC = """# Episode 9 - Manual Verification Checklist

_Generated by `episodes/ep-09-autonomous-agents/preflight.py`. Re-run after substrate changes._
...
- The bot writes to the SAME `lc_statusupdate` table the Coordinator (Ep 8) reads from -- one source of truth, two agents
