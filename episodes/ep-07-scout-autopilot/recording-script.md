...
**Hook-shape:** value-first cold open then proof, mirroring Episode 6 ("We modeled the launch in Dataverse. Today Cowork drives it.").

**Visual (0:00–0:03):** A still of the morning Teams DM the Scout Automation lands every weekday: launch name `Q3 Widget Launch`, two task names, one verbatim risk excerpt. A small `EPISODE 07` badge in the top-left corner from frame 1.

**Visual (0:03–0:07):** Hard cut to a Microsoft Scout chat window mid-`search_data` answer: the result card with a verbatim excerpt from inside the seeded PDF on a `lc_task` row, *"We can't ship this to enterprise with the export crash."*
...
|---|---|---|---|
| **0:00–0:03** | Still: the morning Teams DM from the Scout Automation. Launch name, two task names, one verbatim risk excerpt. | "Our launch playbook lives in Dataverse." | ⬇ **Our launch playbook lives in Dataverse.** |
| **0:03–0:07** | Hard cut to Scout chat, streaming a `search_data` result excerpt from inside the PDF. | "Now Scout runs it for us." | ⬇ **Now Scout runs it for us.** |
| **0:07–0:18** ⭐ **Intro · Why this matters (11s)** | Split: Scout chat on the left mid-answer, Teams DM on the right. Subtle highlight on the verbatim PDF excerpt that appears in both. | "Most launch risks hide in someone's inbox or a PDF nobody opened. Scout now sweeps mail and Teams, dedupes against open tasks, and reads inside the attached files." | ⬇ **The evidence lives inside the PDFs.** |
| **0:18–1:05** ⭐ **Part 1 · The new MCP shape, discovered live, then bottled as a skill (47s)** | Scout chat fullscreen. James pastes the Part 1a discovery prompt; Scout renders the tool catalog inline. Cut tight. James pastes the Part 1b seed prompt (short, no tool names, but with the dedup-via-read-then-read-inside rule called out). Scout drafts the skill body inline; markdown scrolls. James types one iteration. Scout amends. James types "Save it." Tool-use panel fires four MCP calls: `upsert_skill` → `create_skill_resource` → `init_file_upload` → `commit_file_upload`. Hard cut to Power Apps → Skill table → the new `Launch Readiness Sweep` row → the resource attached. | "First, what changed. The Dataverse MCP preview server moved from CRUD per table to sixteen tools across five areas, plus `search_data`, which reads inside attached files. I had Scout introspect it live. Then I asked for a skill that sweeps Outlook and Teams and never files a duplicate. Scout drafted it, we tightened one rule, I said save it. Four MCP calls. The skill is a row in Dataverse." | ⬇ **New shape: 16 tools, 5 areas.** → 0:30 **Plus `search_data`, reads inside files.** → 0:42 **Tell Scout: never file a duplicate.** → 0:55 **Say "save it."** → 1:02 **Skill = row in Dataverse.** |
| **1:05–1:35** ⭐ **Part 2 · Run the skill on Q3, watch `search_data` do dedup, read inside what got attached (30s)** | Scout chat. James pastes `Run the Launch Readiness Sweep against Q3 Widget Launch.` Tool-use panel: `search`/`describe` to load the skill, then the skill executes. The new beat: per finding, **`search_data`** fires once against the LaunchControl model scope and returns matching `lc_task` row paths *plus the excerpt from inside the attached PDF* that triggered the match. That's the dedup moment, one call. Then `update_record` + the file-upload trio attaches the new source to the matched task. Speed-ramp 3x through the panel; drop to 1x on the `search_data` call (zoom on the excerpt block in the result) and on the closing summary, which should call out *N enriched, 0 new*. James pastes the Part 2b question. Answer streams a verbatim quote from inside the newly attached PDF via `file_download`. | "Then I asked one question: do I have any open tasks for export issues on Q3? Scout fired `search_data` once and surfaced three duplicates, plus a line from inside an attached PDF. That's the chaos this skill prevents. Then I ran the sweep. One new task, one enriched, zero new duplicates." | ⬇ **One call. Three duplicates surfaced.** → 1:18 **Run the skill.** → 1:25 **1 new, 1 enriched.** → 1:30 **Answer from inside the new PDF.** |
