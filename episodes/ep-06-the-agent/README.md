...
actions, BYO connectors, and knowledge articles. Episode 6 binds it all to an
agent surface. Twice.
...
[`episodes/ep-06-the-agent/preflight.py`](../../episodes/ep-06-the-agent/preflight.py) is a
two-mode harness:

```powershell
python episodes/ep-06-the-agent/preflight.py --plan       # Markdown plan for review
python episodes/ep-06-the-agent/preflight.py --run        # Execute, print summary
```
...
- **Autonomous behavior.** Triggers, flows, scheduled actions land in
  Episode 7. Today's agent is reactive: the user asks, it answers.
...
6. **Test harness** — `python episodes/ep-06-the-agent/preflight.py --plan` to show
...
| [`episodes/ep-06-the-agent/upload_knowledge.py`](../../episodes/ep-06-the-agent/upload_knowledge.py) | Idempotent uploader: upserts records, uploads files via SDK |
| [`episodes/ep-06-the-agent/setup_table.py`](../../episodes/ep-06-the-agent/setup_table.py) | Idempotent table creator (Ep 3 Part 2) |
| [`episodes/ep-06-the-agent/preflight.py`](../../episodes/ep-06-the-agent/preflight.py) | This episode's substrate harness — `--plan` and `--run` |
...
python episodes/ep-06-the-agent/setup_table.py
python episodes/ep-06-the-agent/upload_knowledge.py

# Verify the substrate is ready
python episodes/ep-06-the-agent/preflight.py --run
