...
Agent calls `episodes/ep-11-agentic-admin/capacity_report.py`, parses the structured output, presents the headline numbers. Three pools (Database, File, Log), per-environment + tenant-wide top-N.
...
For the recording, the agent calls `episodes/ep-11-agentic-admin/agent_blast_radius.py`, which reads four Dataverse tables — `bot`, `botcomponent`, `workflow`, `connectionreference` — and emits a per-agent inventory: actions (MCP-backed flagged), generative-orchestration on/off, external triggers, knowledge sources, plus a tenant-wide connector-usage roll-up.
...
```bash
python episodes/ep-11-agentic-admin/preflight.py
```
...
| Path | Role |
|---|---|
| `episodes/ep-11-agentic-admin/README.md` | This document |
| `episodes/ep-11-agentic-admin/preflight.py` | 6-check preflight harness |
| `scripts/python/seed_pre_q1_status_updates.py` | Backdated cleanup-target seeder |
| `episodes/ep-11-agentic-admin/capacity_report.py` | Capacity beat (#2) — env + tenant-top |
| `episodes/ep-11-agentic-admin/agent_blast_radius.py` | Agent governance beat (#4) — per-agent inventory |
