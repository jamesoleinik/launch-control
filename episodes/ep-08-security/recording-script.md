...
|---|---|---|---|
| **0:00–0:08** ⭐ **Intro · Split-screen · 8s** | Split-screen: left half the RBAC visualizer (persona dropdown, row counts, PII table); right half a Cowork answer with redacted emails. `EPISODE 08` badge top-left from frame 1. | "Last episode: more agents, more risk. Not on Dataverse. Same agent, same question, platform decides what it sees." | ⬇ 0:00 **An AI agent reads my launch data through Dataverse** → 0:04 **Dataverse, not the agent, decides what it can see** |
...
| **1:53–2:00** | End card. *"Next: Episode 9: The Agent."* `github.com/jamesoleinik/launch-control` | "Connect any agent you like. What it can read is the platform's call, not the client's. Next, we point a purpose-built agent at all of it." | ⬇ **Define the security model once in Dataverse; every connected agent inherits it** |
...
| Part 1 | `Row security: one launch query returns 4 tasks for a Member, 12 for an Owner` |
| Part 2 | `Column security: the email is returned masked and the full name is hidden` |
