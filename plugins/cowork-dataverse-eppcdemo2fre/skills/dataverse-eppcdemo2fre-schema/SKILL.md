---
name: dataverse-eppcdemo2fre-schema
description: |
  Discover Dataverse logical names for tables and columns in the EppcDemo2FRE
  environment (https://eppcdemo2fre.crm.dynamics.com/api/mcp_preview). Two entry points:
  ...
  Do NOT use for: querying row data (use dataverse-eppcdemo2fre-mcp), schema
...
# Dataverse Schema Explorer: EppcDemo2FRE

Discovery helper for the **EppcDemo2FRE** Dataverse environment. Resolves
...
row data and never writes. For data queries use the `dataverse-eppcdemo2fre-mcp` skill in
the same plugin.

## When NOT to Use

- Querying actual row data: use `dataverse-eppcdemo2fre-mcp`.
