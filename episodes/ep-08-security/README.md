...
- and a small **impersonation visualizer app**
  ([`apps/rbac-visualizer/`](../../apps/rbac-visualizer/)), built here in Part 1, so
  the model is something you can *see* (and toggle) on screen as each later part
  adds to it.
...
From here every ask is one line, because the skill carries the *how*. We build the
visualizer app here in Part 1 so it's ready as the instrument, then author the
row-level model in Part 2 and the PII masking in Part 3 and watch each one show up
in it.

### Prompt to Cursor: build the visualizer app

Build the instrument first, so every layer you add next is something you can watch
land on screen. One prompt:
...
The persona dropdown fills in once Part 2 creates the roles and teams, and the
masking toggle comes alive once Part 3 adds the rule. Until then the app already
runs and renders the live row counts.
...
### The app you built in Part 1

That smoke-test already proves the row-level contrast in numbers. The **visual**
version is the app you built in Part 1: open it now and the persona dropdown renders
exactly these per-persona counts, plus the per-row table with an owner column, in a
UI you can flip in front of an audience. In Part 3 the same app also carries the
...
### Run the side-by-side masking demo

The visualizer you built in Part 1 reads this environment live, so the masking rule
you just added is the show. Put the app on one side of the screen and Cowork on the
...
| [`episodes/ep-08-security/preflight.py`](preflight.py) | _(planned)_ Read-only check: are the four roles + four teams present, are the sensitive columns secured with the profile bound, is the caller a member of any team, are the `lc_*` tables resolved. |
