...
`scripts/test_ep7_locally.py` is the substrate harness:

```
python scripts/test_ep7_locally.py --plan          # show test plan
python scripts/test_ep7_locally.py --run           # P1-P6 + S1-S2 (read-only)
python scripts/test_ep7_locally.py --trigger       # adds T1 (creates ephemeral task, polls, cleans up)
```
...
Manual UI checklist regenerated each run at `scripts/test_ep7_prompts.md`.
