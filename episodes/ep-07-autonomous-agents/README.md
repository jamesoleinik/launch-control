...
> _"Episode 6's agent answers when you ask. This one acts without you asking. Every time a task gets blocked. Every morning at 8 AM. Same agent. Same data."_

Episode 6 made Dataverse conversational. Episode 7 makes it **proactive** — and shows how a single autonomous agent, bound to the **same Dataverse MCP server**, can act on **two completely different trigger types** without spinning up a second surface.
...
Behavior 1 produces rows in the same `lc_statusupdate` table the Coordinator (Episode 6) reads from. Behavior 2 routes to Teams. Same agent, two effectors, both autonomous.
...
The original plan for Episode 7 had **two surfaces** in the autonomous tier: Sentinel (event-driven bot) and a **separate scheduled Agent Flow** with Dataverse MCP steps that posted the digest. The framing was "MCP tools and MCP flow steps are the same primitive."
...
- [`business-skills/escalation-policy.md`](../../business-skills/escalation-policy.md) — has both an **Interactive mode** section (used by the Coordinator agent in Ep 6) and an **Autonomous mode (Launch Sentinel)** section (used by Sentinel B1). Same skill, two consumers, proving skills are agent-portable.
...
`episodes/ep-07-autonomous-agents/preflight.py` is the substrate harness:

```
python episodes/ep-07-autonomous-agents/preflight.py --plan          # show test plan
python episodes/ep-07-autonomous-agents/preflight.py --run           # P1-P6 + S1-S2 (read-only)
python episodes/ep-07-autonomous-agents/preflight.py --trigger       # adds T1 (creates ephemeral task, polls, cleans up)
```
...
Manual UI checklist regenerated each run at `episodes/ep-07-autonomous-agents/prompts.md`.
...
episodes/ep-07-autonomous-agents/
├── README.md                        # This file
...
- **Same skill, two agents.** `Escalation-Policy` is consumed by the interactive Launch Coordinator (Ep 6, "Interactive mode" section) AND the autonomous Launch Sentinel (Ep 7, "Autonomous mode" section). One source of policy, two completely different runtime shapes. That's the skills-portability claim made concrete.
...
**Episode 8 — The Code-First Agent.** Same skills. Same Dataverse. Same
...
chat agent (Ep 6), the autonomous agent (Ep 7), **and** the Python agent
(Ep 8) all change behavior on their next run. Skills portability, proven
