...
| **0:07–0:18** ⭐ **Intro · The new tool shape (11s)** | Cut between the tool catalog still and `dataverse-mcp-tools.json` open in VS Code. Highlight `search`, `describe`, `read_query`, `file_download`. | "The shape moved from CRUD per table to discovery, query, and files. Search, describe, read. Agents ask in English, the server finds the right thing and the agent can read inside the files it finds." | ⬇ **Search. Describe. Read.** |
...
| **1:05–1:35** ⭐ **Part 2 · Run the skill on Q3, watch dedup work, read inside what got attached (30s)** | Scout chat. James pastes `Run the Launch Readiness Sweep against Q3 Widget Launch.` Tool-use panel: `search`/`describe` to load the skill, then the skill executes. The new beat: per finding, **`read_query` pulls the open `lc_task` candidate set on the launch**, then for an ambiguous candidate **`file_download`** opens the attached PDF so Scout can read inside before deciding (that's the dedup moment), then `update_record` + the file-upload trio attaches the new source to the matched task. Speed-ramp 3x through the panel; drop to 1x on the `file_download` call and on the closing summary, which should call out *N enriched, 0 new*. James pastes the Part 2b question. Answer streams a verbatim quote from inside the newly attached PDF. | "Then I ran it on Q3. For every finding, Scout pulled the launch's open tasks and asked, in context, *does any of these already cover it?* When the title and notes weren't enough, it opened the attached PDF and read inside. Both findings matched. So the new collateral got attached to the existing tasks instead of filing duplicates. No noise added to the queue." | ⬇ **Run the skill.** → 1:13 **Read inside the attached PDF.** → 1:23 **2 enriched, 0 duplicates.** → 1:30 **Answer from inside the new PDF.** |
...
- **What this episode unlocks:** Three wins. (1) The new tool shape is NL-driven. `search` and `describe` let the agent discover the right table or skill instead of being hand-pointed. (2) Files upload directly onto records, and `file_download` pulls those files back into the agent's context on demand, so the agent can read inside an attached PDF before making a decision. (3) Business Skills are first-class Dataverse rows, authored through the same MCP server, so the rules live next to the data and any MCP-aware agent (Scout, Cowork, Claude, the CLI) can find them.
...
Scout introspects the MCP server and renders its tools across the discovery, query, records, tables, business-skills, and files areas. Hold the answer at 1x; the rendered tool list scrolling past is the visual proof that the shape really did change. Cut tight at the moment the list finishes rendering.
...
Tool-use panel sequence: Scout calls `search('launch readiness sweep')` then `describe` to load the skill body, then follows the body. That means: Scout's SharePoint search and read, Scout's Outlook search and read, then on the Launch Control MCP server `read_query` (resolve the launch), `read_query` (pull the open `lc_task` candidate set on the launch — this is the dedup beat: the panel shows Scout reasoning over the returned rows, then optionally calling `file_download` on an ambiguous candidate's `lc_relateddocuments` to read inside before committing), then `update_record` + `init_file_upload` → HTTP PUT → `commit_file_upload` per matched task. Speed-ramp 3x through the panel; the `file_download` beat is the visual proof — hold on it at 1x. Drop to 1x on the closing four-section summary (headline + new + **enriched** + no-ops). The expected mix for the on-camera take is **2 findings, 0 new tasks, 2 enriched**.
...
- [ ] **Launch Control MCP server** registered in Scout: Settings → Extensions → MCP Servers → `https://<your-org>.crm.dynamics.com/api/mcp`. Signed in.
...
- [ ] **`lc_relateddocuments` file column on `lc_task`** enabled. The seeder and the on-camera enrichment both attach to this column. (Per current platform behavior the agent reads inside attachments via `file_download`, not via search-index embeddings.)
