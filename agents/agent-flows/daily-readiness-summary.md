> ## ⚠️ DEFERRED — see [`episodes/ep-09-autonomous-agents/README.md`](../../episodes/ep-09-autonomous-agents/README.md)
>
> The standalone Agent Flow surface was pivoted out of Episode 9 because the preview Dataverse MCP-step UX could not reliably invoke unbound Custom APIs (specifically `lc_CalculateLaunchReadiness`) from natural-language Instructions. We discovered the Instructions box does accept raw Dataverse SQL (preserved in Step 0 / Step 1 below), but Custom API invocation in nested loop steps was the unblockable issue.
>
> **Replacement:** a recurrence trigger added directly to the Launch Sentinel autonomous agent. Same Dataverse MCP server, plus a Teams MCP `SendMessageToSelf` action. See [`agents/launch-sentinel/README.md`](../launch-sentinel/README.md) and [`episodes/ep-09-autonomous-agents/README.md`](../../episodes/ep-09-autonomous-agents/README.md).
>
...
# Daily Readiness Digest — Standalone Agent Flow (Episode 9, Part 2) — DEFERRED
...
> **Scope clarification:** this is an **Agent Flow** (Copilot Studio Flows canvas), not a Power Automate cloud flow. The Dataverse MCP step type is a first-class step in Agent Flows. Don't build this in `flow.microsoft.com` — different surface, different ALM story, and it would defeat the Episode 9 narrative ("MCP tools and MCP flow steps are the same primitive").

> **Why standalone (not inside Sentinel):** the flow doesn't use any of Sentinel's instructions, topics, or knowledge — it's pure deterministic orchestration (list → loop → call API → post). Hosting it inside the bot would be narrative theater, not architecture. The Episode 9 framing is "**the autonomous tier has two surfaces**": Sentinel (event-driven, LLM-reasoning) **and** this flow (cron-driven, deterministic). Both call the **same** Dataverse MCP server. That's the punch line.
...
5. **If no step type appears**:
   ...
   - **Defer Part 2** of Episode 9 to a future re-cut once the step type is GA in your tenant.
...
Why this and not raw queries: the Custom API encapsulates the readiness algorithm (built in Episode 5). Re-implementing it inline in the flow would drift from what the Coordinator (Episode 8) and Sentinel (Episode 9 Part 1) use.
